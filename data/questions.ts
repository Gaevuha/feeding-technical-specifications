...
export const questionsByCategory: Record<Category, QuestionType[]> = {
  ...
  js: [
    ...
    makeQuestion(
      ...
      [
        {
          text: "Closure (замикання) — це функція, яка «пам’ятає» змінні зовнішньої функції навіть після її виконання. Використовується для інкапсуляції даних, збереження стану та створення приватних змінних.",
          correct: true,
        },
        {
    ...
    makeQuestion(
      ...
      [
        {
          text: "Основні принципи ООП: інкапсуляція (приховування даних і доступ через методи), наслідування (об’єкти можуть успадковувати властивості та методи), поліморфізм (об’єкти можуть по-різному реагувати на однакові методи) та абстракція (виділення суттєвих властивостей об’єкта).",
          correct: true,
        },
        {
    ...
    makeQuestion("js-28", "Що таке this і як визначається його значення?", [
      {
        text: "this — це контекст виконання функції; визначається способом виклику функції (метод, звичайний виклик, new, bind/call/apply).",
        correct: true,
      },
    ...
    makeQuestion(
      ...
      [
        {
          text: "call(obj, ...args) — викликає функцію з заданим this; apply(obj, [args]) — те саме, але аргументи як масив; bind(obj, ...args) — створює нову функцію з прив'язаним this; метод об'єкта — this вказує на об'єкт; стрілкові функції — успадковують this зовнішнього контексту.",
          correct: true,
        },
        {
    ...
    makeQuestion("js-30", "Що таке прототип об'єкта і яку роль він відіграє?", [
      {
        text: "Прототип — це об'єкт, який слугує резервним сховищем властивостей і методів для іншого об'єкта; якщо властивість не знайдено в об'єкті, JS шукає її у прототипі.",
        correct: true,
      },
  ...
  react: [
    makeQuestion(
      ...
      [
        {
          text: "JSX — це спеціальний синтаксис у React, який дозволяє описувати інтерфейс за допомогою розмітки, схожої на HTML, безпосередньо у файлах JavaScript або TypeScript. JSX не виконується браузером напряму — під час збірки проєкту він перетворюється (транспілюється) у звичайний JavaScript за допомогою інструментів на кшталт Babel або Vite. Кожен JSX-елемент перетворюється на виклик функції React.createElement, з якої React будує інтерфейс.",
          correct: true,
        },
    ...
    makeQuestion("react-2", "В чому різниця між state і props?", [
      {
        text: "Props — це дані, які передаються з батьківського компонента до дочірнього. Вони є тільки для читання і не можуть змінюватися всередині компонента. Props дозволяють налаштовувати та повторно використовувати компоненти. State — це внутрішній стан компонента, який зберігається всередині нього і може змінюватися. Зміна state призводить до повторного рендеру компонента.",
        correct: true,
      },
      {
    ...
    makeQuestion(
      ...
      [
        {
          text: "key — це спеціальний атрибут у React, який використовується під час рендерингу списків, щоб React міг ідентифікувати кожен елемент і ефективно оновлювати DOM. Key має бути унікальним і стабільним, зазвичай використовується id елемента. Він допомагає React визначити, які елементи були додані, змінені або видалені.",
          correct: true,
        },
    ...
    makeQuestion(
      ...
      [
        {
          text: "Підняття стану (lifting state up) — це механізм у React, за якого спільний стан переміщується з дочірніх компонентів у їхній спільний батьківський компонент, а потім передається назад дочірнім компонентам через props. Це дозволяє кільком компонентам працювати з одними й тими самими даними та залишатися синхронізованими.",
          correct: true,
        },
        {
    ...
    makeQuestion(
      ...
      [
        {
          text: "Refs — це механізм у React, який дозволяє отримати прямий доступ до DOM-елемента або екземпляра компонента, минаючи props і state. Використовуються для фокусу, анімацій, інтеграції DOM-бібліотек і вимірювань.",
          correct: true,
        },
